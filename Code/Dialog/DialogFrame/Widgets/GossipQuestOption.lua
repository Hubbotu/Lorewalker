...
local GossipQuestOption = env.modules:New("@\\Dialog\\DialogFrame\\Widgets\\GossipQuestOption")

local Mixin = Mixin

do -- Option
    local OPTION_PADDING = 8
    local OPTION_ICON_SIZE = 20
    ...
    GossipQuestOption.Option = UIKit.Template(function(id, name, children, ...)
        local frame =
            Frame(name, {
...
do -- Group
    GossipQuestOption.Group = UIKit.Template(function(id, name, children, ...)
        local frame =
            LayoutVertical(name, {
                List(name .. ".OptionListFrame")
                    :id("OptionListFrame", id)
                    :poolTemplate(GossipQuestOption.Option)
